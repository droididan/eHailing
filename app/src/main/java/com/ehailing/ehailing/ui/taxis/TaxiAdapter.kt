...
class TaxiAdapter(val click: (StationModel) -> Unit) : RecyclerView.Adapter<TaxiAdapter.TaxiHolder>() {

    // observe changes on the data model
    var stations: List<StationModel> by Delegates.observable(listOf()) { _, _, _ ->
